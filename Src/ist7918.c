...
//	unsigned char EN8X16[32]
  ...
  write_com(8*y);
	...
	write_com(2*x-2);	   // Start 0000H  
	  ...
	  for(j=0;j<8;j++)
		{
		 write_data(EN8x16[(num*16)+j]);
		}
		for(j=0;j<8;j++)
		{
		 write_data(EN8x16[(num*16)+8+j]);
		}		
}
void LCD_CH16X16(unsigned char x,unsigned char y,uint8_t num)
{
  ...
  write_com(16*y);
	...
	write_com(2*x-2);	   // Start 0000H  
	  ...
	  for(j=0;j<8;j++)
		{
		 write_data(CH16x16[(num*32)+j]);
		}
		for(j=0;j<8;j++)
		{
		 write_data(CH16x16[(num*32)+8+j]);
		}		
		for(j=0;j<8;j++)
		{
		 write_data(CH16x16[(num*32)+16+j]);
		}
		for(j=0;j<8;j++)
		{
		 write_data(CH16x16[(num*32)+24+j]);
		}
}
void LCD_EN16X32(unsigned char x,unsigned char y,uint8_t num)
{
  ...
  write_com(16*y);
	...
	write_com(4*x-2);	   // Start 0000H  
	  ...
	  for(j=0;j<8;j++)
		{
		 write_data(EN16x32[(num*64)+j]);
		}
		for(j=0;j<8;j++)
		{
		 write_data(EN16x32[(num*64)+8+j]);
		}		
		for(j=0;j<8;j++)
		{
		 write_data(EN16x32[(num*64)+16+j]);
		}
		for(j=0;j<8;j++)
		{
		 write_data(EN16x32[(num*64)+24+j]);
		}
    for(j=0;j<8;j++)
		{
		 write_data(EN16x32[(num*64)+32+j]);
		}

		for(j=0;j<8;j++)
		{
		 write_data(EN16x32[(num*64)+40+j]);
		}
    for(j=0;j<8;j++)
		{
		 write_data(EN16x32[(num*64)+48+j]);
		}
    for(j=0;j<8;j++)
		{
		 write_data(EN16x32[(num*64)+56+j]);
		}
}
//========================================================
...
uint8_t Addr=0,thshold=0,channel=0,Sleep_Time=0,flag_send=0;//地址，阈值，信道，休眠时间,发送标志位
uint8_t addr_num,thshold_num,channel_num,sleep_num;         //地址，阈值
...
//				  HAL_Delay(1000);
	...
	if(line==1)
	{
		if(temp<=9)                     //输入数字键
		{
		 ...
		 switch(addr_num)
			{	
			case  1:Addr=temp;break;                //第一次输入
			case  2:Addr=temp+Addr*10;break;        //第二次输入
			}
		 if(addr_num<=1)      addr_num=1;         //限幅位数
		 else if( addr_num>=2)addr_num=2;

			if( Addr>99){Addr=99;addr_num=2;}
			else if(Addr<=0){addr_num=0;Addr=0;}
		}
	...
	else if(line==2)
	{
		if(temp<=9)                     //输入数字键
		{
			 ...
			 switch(channel_num)
				{	
				case  1:channel=temp;break;                   //第一次输入
				case  2:channel=temp+channel*10;break;        //第二次输入
				}
			 if(channel_num<=1)      channel_num=1;         //限幅位数
			 else if( channel_num>=2)channel_num=2;

				if( channel>255){channel=83;channel_num=2;}
				else if(channel<=0){channel_num=0;channel=0;}
		}
		else if (temp==11)   //删除键del
		{
			channel_num--;
			channel=channel/10;
			if(channel==0)	channel_num=0;		 
		}
	}
	else if(line==3)              //电阻阈值
	{
	 if(temp<=9)
		{
		 ...
		 switch(thshold_num)
			{	
			case  1:thshold=temp;break;                //第一次输入
			case  2:thshold=temp+thshold*10;break;     //第二次输入
			}
		 if(thshold_num<=1)     thshold_num=1;          //限幅位数
		 else if( thshold_num>=2)thshold_num=2;

		 if( thshold>99){thshold=99;thshold_num=2;}
		 else if(thshold<=0){thshold_num=0;thshold=0;}
		}
		else if (temp==11)   //删除键del
		 {
			thshold_num--;
	...
	else if(line==4)         //休眠时间
	{
		if(temp<=9)   //
		{
		 ...
		 switch(sleep_num)
			{	
			case  1:Sleep_Time=temp;break;                   //第一次输入
			case  2:Sleep_Time=temp+Sleep_Time*10;break;     //第二次输入
			}
		 if(sleep_num<=1)     sleep_num=1;                   //限幅位数
		 else if( sleep_num>=2)sleep_num=2;

		 if( Sleep_Time>24){Sleep_Time=24;sleep_num=2;}
		 else if(Sleep_Time<=0){sleep_num=0;Sleep_Time=0;}
		}
		else if (temp==11)   //删除键del
		 {
			Sleep_Time--;
...
//	   LCD_CH16X16(2,4,5); //道
//		 LCD_EN8X16(2,10,11); 
//		 LCD_EN8X16(2,12,channel/10); 	
//		 LCD_EN8X16(2,13,channel%10); 
//			
//		 LCD_CH16X16(3,1,0);//电
//	   LCD_CH16X16(3,2,1);//阻
//	   LCD_CH16X16(3,3,6);//阈
...
void Resistance_Display(float Resistance ,uint8_t page)//电阻显示
{
  ...
  Addr=remote_buf[0];//获取地址
	...
	thshold=remote_buf[2];//获取阈值
	Sleep_Time=remote_buf[3];//获取休眠时间
	if(Addr>255&&channel>84&&thshold>99&&Sleep_Time>24)
	{
		Addr=1;
		channel=23;
		thshold=10;
		Sleep_Time=1;
		remote_data(Addr,channel,thshold,Sleep_Time);//FLASH中写入数据			
	}
	   LCD_CH16X16(4,0,2);//地
     LCD_CH16X16(4,1,3);//址
	   LCD_EN8X16(4,4,11); //:
	   ...
	   LCD_CH16X16(4,5,5);//道
	   LCD_EN8X16(4,12,11); //:
		 LCD_EN8X16(4,13,channel/10); //
	   LCD_EN8X16(4,14,channel%10); //
